package com.epsilon.lx.fastdfs.pool;
...
import com.epsilon.lx.exception.GlobalException;
            ...
            if (nowPoolSize < maxPoolSize) {
                ...
                try {
                    ...
                    log.error("[获取空闲连接(checkout)-error][" + logId
                            + "][error:获取连接超时:{}]",e);
                    throw new FastDFSException(ErrorCode.WAIT_IDLECONNECTION_TIMEOUT);
                }
            }
            if (trackerServer == null) {
                log.error("[获取空闲连接(checkout)-error][" + logId
